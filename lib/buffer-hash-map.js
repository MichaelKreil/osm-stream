...
function BufferHashMap(opt) {
	...
	function loadBufferHashMap(filename) {
		...
		function init() {
			...
			indexCount = buffer.readUInt32LE(0);
			entryCount = buffer.readUInt32LE(4);
			valueBytes = buffer.readUInt32LE(8);
			...
			values = buffer.slice(headerSize+lookupSize);
			//console.log(indexCount, entryCount, valueBytes, headerSize, lookupSize, valuesSize);
			if (values.length !== valuesSize) throw Error();
		}

		function get(hash) {
			let lookupIndex = hash*6;
			let valueCount = lookup.readUInt16LE(lookupIndex);
			if (valueCount === 0) return false;

			let valueIndex = lookup.readUInt32LE(lookupIndex+2);
	...
	function createBufferHashMap(opt) {
		...
		function save(filename) {
			...
			// write header
			buffer.writeUInt32LE(indexCount, 0);
			buffer.writeUInt32LE(entryCount, 4);
			buffer.writeUInt32LE(valueBytes, 8);
